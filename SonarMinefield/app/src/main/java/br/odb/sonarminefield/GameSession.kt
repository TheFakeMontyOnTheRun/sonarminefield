...
class GameSession {
	...
	fun placeRandomMines(numMines: Int) {
		var n = numMines
		var x: Int
		var y: Int
		val random = Random()
	...
	fun poke(x: Int, y: Int): GameBoard.GameOutcome {
		...
		if (x < 0 || x >= width || y < 0 || y >= height) return GameBoard.GameOutcome.PLAYING

		if (flagged[y]!![x]) {
			flagged[y]!![x] = false
			return GameBoard.GameOutcome.PLAYING
		}
		when (map[y]!![x]) {
			POSITION_MINE -> {
				map[y]!![x] = POSITION_MINE_POKED
				return GameBoard.GameOutcome.LOST
			}
			POSITION_BLANK -> {
				floodUncover(x, y)
		...
		if (remainingTilesToClear == mines) return GameBoard.GameOutcome.WON
